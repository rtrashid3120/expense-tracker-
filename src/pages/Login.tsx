...
export function Login() {
  ...
  const handleDemoLogin = async () => {
    ...
    try {
      try {
        await api.login('demo@expensehub.app', 'demo123456');
      } catch (e) {
        await api.signup('demo@expensehub.app', 'demo123456', 'rashid_demo', 'Mohamed Rashid');
      }
      await initAuth();
    } catch (err: any) {
      ...
      >
        <div className="glass-panel p-8 md:p-12 relative z-10">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full bg-gradient-to-r from-blue-600 to-purple-600 dark:from-brand-neon dark:to-brand-purple text-white dark:text-black font-black text-base rounded-2xl px-4 py-4 mb-3 shadow-md hover:shadow-lg transition-all active:scale-[0.98] disabled:opacity-70 flex items-center justify-center gap-2 cursor-pointer"
          >
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-700 dark:text-white font-bold text-base rounded-2xl px-4 py-3.5 mb-6 shadow-sm hover:shadow-md dark:hover:bg-white/10 flex items-center justify-center gap-3 transition-all active:scale-[0.98] disabled:opacity-70 disabled:active:scale-100"
          >
